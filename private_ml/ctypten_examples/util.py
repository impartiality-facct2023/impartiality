...
def kmeans_inference(data, clusters, hard=True, bandwidth=1.0):
    ...
    distances = data.matmul(clusters.t()).mul(-2.0).add_(d2_sum).add_(
        c2_sum.t())
...
def kmeans(data, K, max_iter=100):
    ...
    for iter in range(max_iter):
        ...
        if prev_assignments is not None:
            ...
            logging.info(
                "K-means iteration %d: %d assignments changed" % (
                iter, num_changes)
            )
            if num_changes == 0:
                break
...
def pca(data, components):
    ...
    """
    assert components > 0 and components < data.size(
        1), "incorrect # of PCA dimensions"
