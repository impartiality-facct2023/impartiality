...
class CelebADataset(VisionDataset):
    ...
    def _check_integrity(self):
        for (_, md5, filename) in self.file_list:
            self.root = '/scratch/ssd002/datasets/celeba_pytorch/'
            ...
            if ext not in [".zip", ".7z"] and not check_integrity(fpath, md5):
                ...
                computed_md5 = calculate_md5(fpath=fpath)
                print('computed_md5: ', computed_md5)
                return False
...
class CelebASensitive(CelebADataset):

    def __init__(self, root, split="train", target_type="attr", target_feat="Smiling", sensitive_feat="Male", transform=None, target_transform=None, download=False):
    ...
    def __getitem__(self, index):
        X, target = super(CelebASensitive, self).__getitem__(index)
        # return X, torch.nn.functional.one_hot(target[self.target_feat_col_id].to(torch.int64), num_classes=2), target[self.sensitive_feat_col_id]
...
if __name__ == "__main__":
    ...
    celeba = CelebADataset(root=f'/home/{user}/data/celeba/')
