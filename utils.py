...
from datasets.fairface.fairface_utils import get_fairface_private_data, get_fairface_test_set, get_fairface_balanced_private_data
from datasets.utkface.utkface_utils import get_utkface_private_data, get_utkface_test_set, get_utkface_balanced_private_data, get_utkface_train_set
from datasets.gaussian.gaussian_utils import get_gaussian_private_data, get_gaussian_test_set
from datasets.chexpert.chexpert_utils import get_chexpert_sensitive_private_data, get_chexpert_sensitive_test_set, get_chexpert_sensitive_unlabeled_set, get_chexpert_sensitive_train_set
...
from fairness.losses import CEWithDemParityLoss, CEWithDemParityLossPub
'''
...
from queryset import QuerySet, get_sensitives_filename
...
def save_raw_queries_targets(args, dataset, indices, name):
    ...
    for data, *targets in queryloader:
        ...
        all_sensitives.append(sensitive.numpy())

    
    if args.dataset == "gaussian":
        all_samples = np.concatenate(all_samples, axis=0)
        assert len(all_samples.shape) == 2 and all_samples.shape[0] == len(indices)
    else:
    ...
    if "mnist" in args.dataset and args.dataset != "colormnist":
    ...
    save_sensitives(name=name, args=args, sensitives=all_sensitives)
...
def save_sensitives(args, name, sensitives):
    
    sensitives = np.concatenate(sensitives, axis=0)
    filename = get_sensitives_filename(name=name, args=args)
    filepath = os.path.join(args.ensemble_model_path, filename)
    np.save(filepath, sensitives)
...
def get_all_targets(dataloader, args) -> Optional[Tensor]:
        ...
        for _, *targets in dataloader:
            ...
            if all_targets is None:
                if len(targets.shape) == 1:
                    ...
                    all_sensitive = torch.zeros(dataset_len)
                elif len(targets.shape) == 2:
                    num_labels = targets.shape[1]
                    all_targets = torch.zeros((dataset_len, num_labels))
                    all_sensitive = torch.zeros((dataset_len, num_labels))
                else:
                    raise Exception(
            ...
            all_sensitive[begin:end] += sensitive
    if args.has_sensitive_attribute:
        return all_targets, all_sensitive
    else:
        return all_targets, None


def get_all_targets_numpy(dataloader, args) -> Optional[np.ndarray]:
    all_targets, sensitive = get_all_targets(dataloader=dataloader, args=args)
...
def load_private_data_imbalanced(args):
    ...
    if "mnist" in args.dataset:
        ...
        return get_fairface_balanced_private_data(args)
    elif args.dataset == "utkface":
        return get_utkface_balanced_private_data(args)
    else:
        raise Exception(args.datasets_exception)
...
def load_private_data(args):
    ...
    kwargs = get_kwargs(args=args)
    args.kwargs = kwargs
    if args.dataset in ["mnist", "colormnist", "fashion-mnist"]:
        return get_mnist_private_data(args=args)
    elif args.dataset == "svhn":
        return get_svhn_private_data(args=args)
    elif args.dataset.startswith("cifar"):
        return get_cifar_private_data(args=args)
    elif args.dataset.startswith("chexpert") and args.dataset != "chexpert-sensitive":
        return get_chexpert_private_data(args=args, kwargs=kwargs)
    elif args.dataset == "retinopathy":
        return get_retinopathy_private_data(args=args)
    elif args.dataset in ["celeba", "celebasensitive"]:
        return get_celeba_private_data(args=args)
    elif args.dataset  == "fairface":
        return get_fairface_private_data(args=args)
    elif args.dataset  == "utkface":
        return get_utkface_private_data(args=args)
    elif args.dataset  == "gaussian":
        return get_gaussian_private_data(args=args)
    elif args.dataset  == "chexpert-sensitive":
        cfg = get_cfg(os.path.join(os.getcwd(),"capc-learning/datasets/chexpert/single_disease_small.json"))
        return get_chexpert_sensitive_private_data(args=args, kwargs=kwargs, cfg=cfg)
...
def get_train_dataset(args):
    '''
    Loads the whole train dataset and then select a subset that satsify the fairness constraint if required
    '''
    if "mnist" in args.dataset:
        dataset = get_mnist_dataset(args=args, train=True)
    elif args.dataset == "utkface":
        dataset =  get_utkface_train_set(args)
    elif args.dataset == 'chexpert-sensitive':
        cfg = get_cfg(os.path.join(os.getcwd(),"capc-learning/datasets/chexpert/single_disease_small.json"))
        return get_chexpert_sensitive_train_set(args=args, cfg=cfg)
    
    start = 0
    end = len(dataset) - args.num_val_samples
    dataset =Subset(dataset=dataset, indices=list(range(start, end)))
    ...
    for index in range(len(dataset)):
        ...
        if per_class_pos_classified_group_count[int(pred), z] < args.min_group_count:
            ...
            if _new_gaps[int(pred), z] + np.random.normal(0., 0.0) <= args.max_fairness_violation:
                ...
                per_class_pos_classified_group_count[int(pred), z] += 1
                indices_selected.append(index)
    dataset =Subset(dataset=dataset, indices=indices_selected)
    return dataset
...
def get_non_train_set(args):
    ...
    if "mnist" in args.dataset:
        ...
        dataset = get_gaussian_test_set(args=args)
    elif args.dataset == "coco":
...
def get_val_set(args):
    """
    Get the validation set by taking it from the train data
...
def get_test_set(args):
    ...
    if args.dataset in args.xray_datasets:
        ...
        cfg = get_cfg(os.path.join(os.getcwd(),"capc-learning/datasets/chexpert/single_disease_small.json"))
        dataset = get_chexpert_sensitive_test_set(args=args, cfg=cfg)
        return dataset
...
def get_unlabeled_set(args):
    ...
    if args.dataset in args.xray_datasets:
        ...
        cfg = get_cfg(os.path.join(os.getcwd(),"capc-learning/datasets/chexpert/single_disease_small.json"))
        dataset = get_chexpert_sensitive_unlabeled_set(args=args, cfg=cfg)
        return dataset
...
def get_loss_criterion(model, args):
    ...
    elif args.loss_type == "CEWithDemParityLoss":
        if not args.has_sensitive_attribute:
            raise Exception("Needs to have `has_sensitive_attribute=true`")
        criterion = CEWithDemParityLoss(_lambda=args.inprocessing_fairness_penalty_scaler, _gamma=args.max_fairness_violation)
    elif args.loss_type == "CEWithDemParityLossPub":
        if not args.has_sensitive_attribute:
            raise Exception("Needs to have `has_sensitive_attribute=true`")
        criterion = CEWithDemParityLossPub(_lambda=args.inprocessing_fairness_penalty_scaler, _gamma=args.max_fairness_violation)
    else:
...
def compute_loss(target, output, criterion, weights, args, model, data, sensitive=None, output_pub=None, sensitive_pub=None, DPLoss=None):
    """
    Compute the loss.
    ...
    if args.dataset in args.xray_datasets:
        if weights is None:
            raise Exception("`weights` cannot be None")
        loss = task_loss(
        ...
        loss = add_regularization_loss(args=args, model=model, data=data,
                                       loss=loss)
        return loss
    elif args.dataset in ["colormnist", "celebasensitive", "fairface", "utkface", "chexpert-sensitive"]:
        
        targ = F.one_hot(target.to(torch.int64), num_classes=args.num_classes).float()
        
        if args.dataset == 'chexpert-sensitive':
            targ = torch.squeeze(targ)
        if args.loss_type == 'CEWithDemParityLossPub':
            
            if DPLoss != None:
                loss, _ = criterion(input=output, target=targ, input_pub=None, sensitive_pub=None, DP_loss=DPLoss)
            else:
                sens_pub = F.one_hot(sensitive_pub.to(torch.int64), num_classes=max(args.sensitive_group_list)+1).float()
                if args.dataset == 'chexpert-sensitive':
                    sens_pub = torch.squeeze(sens_pub)
                loss, DPLoss = criterion(input=output, target=targ, input_pub=output_pub, sensitive_pub=sens_pub)
                return loss, DPLoss
        elif args.inprocessing_fairness:
            sens = F.one_hot(sensitive.to(torch.int64), num_classes=max(args.sensitive_group_list)+1).float()
            if sensitive is None:
                raise Exception('`sensitive` cannot be none if `inprocessing_fairness=true`')
            if args.dataset == 'chexpert-sensitive':
                sens = torch.squeeze(sens)
            loss = criterion(output, targ, sens)
        else:
...
def train(model, trainloader, optimizer, criterion, args, public_dataloader=None):
    ...
    for batch_id, (data, *target) in enumerate(trainloader):
        ...
        if public_dataloader:
            
            output_pub = torch.tensor([]).cuda()
            sensitive_pub = torch.tensor([]).cuda()
            for (data, *target_pub) in public_dataloader:
                _, sens_pub = target_pub
                if args.cuda:
                    data = data.cuda()
                    sens_pub = sens_pub.cuda()
                out_pub = model(data)
                sensitive_pub = torch.cat((sensitive_pub, sens_pub))
                output_pub = torch.cat((output_pub, out_pub))
            if args.cuda:
                output_pub, sensitive_pub = output_pub.cuda(), sensitive_pub.cuda()
            output_pub, sensitive_pub = output_pub.to(torch.float32), sensitive_pub.to(torch.float32)
            model.train()
            
            loss, DPLoss = compute_loss(
                target=target,
                sensitive=sensitive,
                output=output,
                criterion=criterion,
    ...
    with torch.no_grad():
        for data, *target in dataloader:
            ...
            if args.inprocessing_fairness:
                loss = compute_loss(target=target, output=output, sensitive=sensitive, criterion=criterion, weights=None, args=args, model=model, data=data, DPLoss=DPLoss)
            else:
                loss = compute_loss(target=target, output=output, criterion=criterion, weights=None, args=args, model=model, data=data)
            losses.append(loss.item())
            if args.dataset == "gaussian":
                preds = output.round()
            else:
...
def train_model(args, model, trainloader, evalloader, patience=None, private=False, public_dataloader=None):
    device, device_ids = get_device(args=args)
    DPLoss = None
    if private:
...
def save_model(args, model, epoch=-1, result_test=None):
    ...
    if save_model_path is not None:
        ...
        state["epoch"] = epoch
        filename = "checkpoint-{}.pth.tar".format("resnet50")  
        filepath = os.path.join(save_model_path, filename)
...
def get_bin(x: int, n: int) -> str:
    """
    Get n-bit representation of number x.
...
def generate_n_choose_k(n, k):
    """
    Generate all n choose k binary vectors.
...
def fill_v_key_count_powerset_tau(num_labels: int, tau: int) -> (np.ndarray, dict, dict):
    """
    Map indexes (simple 0,1,2, etc.) to the strings representing the powerset
    labels. The votes are for indexes and then we can re-map from the winning
    index to the binary vector of {0,1} labels.
    ...
    Args:
        num_labels: the number of labels
        tau: max number of positive labels

    Returns:
        ordered all possible class labels and a dictionaries with mappings
        between the class indexes and class labels.
    ...
    idx = 0  
    for num_pos in range(tau + 1):
        vectors = generate_n_choose_k(n=num_labels, k=num_pos)
        for vector in vectors:
            str_label = to_str(vector)
            class_labels.append(str_label)
            
            map_idx_label[idx] = str_label
            map_label_idx[str_label] = idx
            idx += 1
    return np.array(class_labels), map_idx_label, map_label_idx


def transform_votes_powerset(input_votes: np.ndarray) -> (np.ndarray, dict):
    """
...
def generate_histograms_powerset(input_votes: np.ndarray,
    ...
    Returns: histograms with histogram for each sample.
    """
...
def get_multilabel_powerset_filenames(args):
    ...
    if args.class_type in ['multilabel_powerset_tau']:
        if args.powerset_tau is None:
            powerset_tau = '-1'
        else:
            powerset_tau = args.powerset_tau
        prefix += f"_powerset_tau_{powerset_tau}"
...
def get_class_labels_and_map_powerset(args, num_labels,
    ...
    if os.path.isfile(class_labels_filename):
        ...
        print(f"Elapsed time to generate map_idx_label: {stop - start} (sec).")
    else:
        start = time.time()
        if args.class_type == 'multilabel_powerset':
            class_labels, map_idx_label = fill_v_key_count_powerset(
                num_labels=num_labels)
        elif args.class_type == 'multilabel_powerset_tau':
            class_labels, map_idx_label, _ = fill_v_key_count_powerset_tau(
                num_labels=num_labels, tau=args.powerset_tau)
        stop = time.time()
...
def pick_votes_from_probabilities(probs, powerset_tau, threshold):
    """
    For each ballot (the binary vector from a teacher, set as positive up to
    powerset_tau labels based on the top-powerset_tau (highest) probabilities.

    Args:
        probs: the probability per label, shape (num_queries, num_teacher, num_labels)
        powerset_tau: the max positive labels in the ballot
        threshold: the probability threshold for a positive label

    Returns:
        tau clipped votes for powerset

    """
    
    probs = torch.from_numpy(probs)
    topk, indices = torch.topk(probs, k=powerset_tau)
    
    votes = torch.zeros_like(probs)
    
    votes = votes.scatter(dim=-1, index=indices, src=topk)
    
    votes = votes >= torch.tensor(threshold)
    
    votes = votes.to(torch.int)
    return votes.numpy()
